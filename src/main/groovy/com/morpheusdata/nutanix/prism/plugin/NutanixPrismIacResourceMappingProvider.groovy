...
class NutanixPrismIacResourceMappingProvider implements IacResourceMappingProvider {
	 ...
	 * inline `name`/`value` string pairs. CategoriesSync on this branch still keys MetadataTag
	 * by display name, so matching v2 tags by ext_id is currently a no-op until category sync
	 * stores V4 extIds. Collecting ext_id here keeps the binding path ready for that migration.
	 */
